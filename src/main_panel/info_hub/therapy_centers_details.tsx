...
import { Layout, Button, Typography, Card, Input, List, message } from 'antd';
...
const TherapyCentersDetails = () => {
  ...
  useEffect(() => {
    axios.get(`https://project-back-81mh.onrender.com/info-hub/infohub/${id}/`)
      ...
      .catch(() => message.error('Failed to load therapy center'));
  }, [id]);
        ...
        <Content className="details-container">
          ...
          {data && (
            <Card className="details-card">
              ...
              <Text>Tags: {data.tags.map((t: any) => t.name).join(', ')}</Text>
            </Card>
          )}
